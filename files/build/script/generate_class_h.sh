...
# ====================== 配置项 ======================
CONFIG_FILE="./.config"
OUTPUT_HEADER="./wt_tools_class.h"
CLASS_STRUCT_TYPE="struct hw_class"

# ====================== 用法说明 ======================
usage() {
    echo "用法: $0 [选项]"
    echo "  功能：从.config提取CONFIG_CLASS_*=y，自动生成全局类实例+映射数组"
    echo "  选项："
    echo "    -c <path>  指定.config文件路径（默认：./.config）"
    echo "    -o <path>  指定输出头文件路径（默认：./wt_tools_class.h）"
    echo "    -t <type>  指定类结构体类型（默认：struct hw_class）"
    echo "    -h         显示帮助信息"
    exit 1
}
...
MATCH_LINES=$(grep -E '^[[:space:]]*CONFIG_CLASS_[A-Z0-9_]+[[:space:]]*=[[:space:]]*y[[:space:]]*$' "$CONFIG_FILE")

if [ -z "$MATCH_LINES" ]; then
    echo "⚠️ 警告：.config中未找到CONFIG_CLASS_*=y格式的配置项"
    CLASS_INSTANCES=""
    CLASS_ENTRIES=""
else
    echo "🔍 匹配到的CONFIG_CLASS配置项："
...
#ifndef __WT_TOOLS_CLASS_H
#define __WT_TOOLS_CLASS_H
//#include "core.h"
...
if [ -z "$CLASS_INSTANCES" ]; then
    echo "   无CONFIG_CLASS_*=y配置项，未生成任何类实例"
else
    echo -e "   生成的全局类实例："
