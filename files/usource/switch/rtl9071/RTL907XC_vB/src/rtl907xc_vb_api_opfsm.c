...
static const uint32 cvb_g_sysPortToLocalCmd[CVB_RTK_SYS_PORT_ID_MAX] = {0x920u, 0x922u, 0x924u, 0x926u, 0x928u, 0xA4Cu, 0xA52u, 0xA54u, 0x0u, 0xA4Eu, 0xA50u, 0x0u};
...
static const uint32 cvb_g_sysPortToLocalPxCmd[CVB_RTK_SYS_PORT_ID_MAX] = {0x826u, 0x828u, 0x82Au, 0x82Cu, 0x82Eu, 0x830u, 0x836u, 0x838u, 0x0u, 0x832u, 0x834u, 0x0u};
/* For RTL907X_C Wake-out Forwarding. The mapping from API parameter "configIdx" to GPIO index */
static const uint8 cvb_g_wakeoConfigIdxToGpioIdx[2] = {OP_WAKEO_GPIOB1_ID, OP_WAKEO_GPIOB2_ID};
 ...
 * @param[out] pPortMap Pointer to the sleep capability port map (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 *
 * @retval RT_ERR_OK on success.
 * @retval -RT_ERR_NULL_POINTER if the pointer *pPortMap is NULL.
 * @retval Other error codes as defined in cvb_reg_read().
 *
 * @note The PHY of port7, port8, port9, port10, port11, and port12 do not support sleep capability configuration.
 *
 * @see cvb_op_set_phy_sleep_cap(uint32 unit, RtkPmap portMap, uint8 sleepCapOn).
 */
RtkApiRet cvb_op_get_phy_sleep_cap(uint32 unit, RtkEnable sleepCap, RtkPmap *pPortMap)
{
    uint32    regOffset = 0u;
    uint32    regValue  = 0u;
    RtkPort   userPort  = 0u;
    ...
    do
    {
        ...
        /* Check input parameters */
        if((CVB_ENABLED != sleepCap) && (CVB_DISABLED != sleepCap))
        {
            retVal = -RT_ERR_INPUT;
            continue;
        }
        /* get sleep capability of all TC10-compatible ports */
        for(userPort = CVB_OP_T1_USER_PORT_MIN; userPort <= CVB_RTK_USER_PORT_WITHT1PHY_ID_MAX; userPort++)
        {
            /* check the PHY type of each port */
#ifdef CVB_OP_RTPGE_USER_PORTMAP
            if(0x0u != (CVB_OP_RTPGE_USER_PORTMAP & (1u << userPort)))
            {
                /* get 1000Base-T1 sleep capability */
                regOffset = ((uint32)(cvb_g_sysPortToOcpAddr[cvb_g_userPort2sysPort[userPort]] | CVB_OCP_SLP_CAP_GIGAPHY)) << 1u;
            }
#endif
#ifdef CVB_OP_BWPHY_USER_PORTMAP
            else if(0x0u != (CVB_OP_BWPHY_USER_PORTMAP & (1u << userPort)))
            {
                /* get 100Base-T1 sleep capability */
                regOffset = ((uint32)(cvb_g_sysPortToOcpAddr[cvb_g_userPort2sysPort[userPort]] | CVB_OCP_SLP_CAP_100T1)) << 1u;
            }
#endif
            else
            {
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[in] portMap Sleep capability port map (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert(), cvb_drv_phy_1000mt1_phy_sleep_cap_set(), and cvb_drv_phy_100mt1_phy_sleep_cap_set().
 *
 * @note The PHY of ports 7, 8, 9, 10, 11, and 12 cannot be in sleep mode.
 */
RtkApiRet cvb_op_set_phy_sleep_cap(uint32 unit, RtkPmap portMap, RtkEnable sleepCap)
{
    uint32    regOffset    = 0u;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert(portMap, &dstPmap, CVB_EM_USER_TO_T1PHY));
        /* Search the user port to be configured according to the input portMap */
        for(userPortId = CVB_OP_T1_USER_PORT_MIN; userPortId <= CVB_RTK_USER_PORT_WITHT1PHY_ID_MAX; userPortId++)
        {
...
#ifdef CVB_OP_RTPGE_USER_PORTMAP
            if(0x0u != (CVB_OP_RTPGE_USER_PORTMAP & (1u << userPortId)))
            {
                /* 1000Base-T1 */
                regOffset    = ((uint32)(cvb_g_sysPortToOcpAddr[cvb_g_userPort2sysPort[userPortId]] | CVB_OCP_SLP_CAP_GIGAPHY)) << 1u;
                regOAMOffset = ((uint32)(cvb_g_sysPortToOcpAddr[cvb_g_userPort2sysPort[userPortId]] | CVB_OCP_OAM_CAP_GIGAPHY)) << 1u;
                CHK_FUN_CONTINUE(retVal, cvb_drv_phy_1000mt1_phy_sleep_cap_set(unit, regOffset, regOAMOffset, sleepCap));
            }
#endif
#ifdef CVB_OP_BWPHY_USER_PORTMAP
            else if(0x0u != (CVB_OP_BWPHY_USER_PORTMAP & (1u << userPortId)))
            {
                /* 100Base-T1 */
                regOffset = ((uint32)(cvb_g_sysPortToOcpAddr[cvb_g_userPort2sysPort[userPortId]] | CVB_OCP_SLP_CAP_100T1)) << 1u;
                CHK_FUN_CONTINUE(retVal, cvb_drv_phy_100mt1_phy_sleep_cap_set(unit, regOffset, sleepCap));
            }
#endif
            else
            {
 ...
 *        For example, if a port is not configured to accept received sleep requests and the sleep capability is enabled (by API set_cvb_op_phy_sleep_cap),
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[in] portMap The port map indicating selected ports in AcceptSleepCmdMsk to be configured (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert(), cvb_drv_indirect_sram_2bytes_get(), and cvb_drv_indirect_sram_2bytes_set().
 *
 * @note References:
 *       - cvb_op_set_phy_sleep_cap(uint32 unit, RtkPmap portMap, RtkEnable sleepCapOn)
 *       - cvb_op_set_txsleep_portmap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue)
 */
RtkApiRet cvb_op_set_accept_sleep_req_cap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue)
{
    uint16    value16 = 0u;
    RtkPmap   dstPmap = 0u;
    RtkPort   phyId   = 0u;
    ...
    {
        /* Check input parameters */
        if((CVB_ENABLED != cfgValue) && (CVB_DISABLED != cfgValue))
        {
            retVal = -RT_ERR_INPUT;
            continue;
        }
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_get(unit, CVB_OP_ACCEPT_SLEEP_CAP, &value16));

        /* Update the accept sleep capability setting */
        for(phyId = 0u; phyId <= CVB_RTK_PHY_ID_MAX; phyId++)
        {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_set(unit, CVB_OP_ACCEPT_SLEEP_CAP, value16));
    } while(0u == 1u);
 ...
 *                     Enable or disable the accept sleep capability.
 * @param[out] pPortMap Pointer to the AcceptSleepCmdMsk (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval RT_ERR_OK on success.
 * @retval -RT_ERR_NULL_POINTER if the pointer *pPortMap is NULL.
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert() and cvb_drv_indirect_sram_2bytes_get().
 *
 * @note Reference:
 *       - cvb_op_set_accept_sleep_req_cap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue).
 */
RtkApiRet cvb_op_get_accept_sleep_req_cap(uint32 unit, RtkEnable cfgValue, RtkPmap *pPortMap)
{
    uint16    value16 = 0u;
    RtkPmap   srcPmap = 0u;
    RtkPmap   dstPmap = 0u;
    ...
    do
    {
        ...
        if(CVB_DISABLED == cfgValue)
        {
            value16 = ~value16;
        }
        srcPmap = (RtkPmap)value16;
        srcPmap &= CVB_RTK_T1PHY_PMAP_ALL;
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert(srcPmap, &dstPmap, CVB_EM_T1PHY_TO_USER));
...
/**
 * @brief Set specific ports (TXSleepPortMsk) to transmit sleep requests when any other port on the RTL907X_C receives a sleep request.
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[in] portMap The port map indicating selected ports in TXSleepPortMsk to be configured (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 *       - cvb_op_set_accept_sleep_req_cap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue).
 */
RtkApiRet cvb_op_set_txsleep_portmap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue)
{
    uint16    value16 = 0u;
    RtkPmap   dstPmap = 0u;
    RtkPort   phyId   = 0u;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_get(unit, CVB_OP_TXSLEEP_PORTMASK, &value16));
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_set(unit, CVB_OP_TXSLEEP_PORTMASK, value16));
    } while(0u == 1u);
 ...
 * @param[out] pPortMap Pointer to the TXSleepPortMsk (Range: 0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_indirect_sram_2bytes_get() and cvb_drv_t1phy_pmap_convert().
 *
 * @note Reference:
 *       - cvb_op_set_txsleep_portmap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue).
 */
RtkApiRet cvb_op_get_txsleep_portmap(uint32 unit, RtkEnable cfgValue, RtkPmap *pPortMap)
{
    uint16    value16 = 0u;
    RtkPmap   srcPmap = 0u;
    RtkPmap   dstPmap = 0u;
...
/**
 * @brief Allow or disallow the RTL907X_C to enter WHOLE_CHIP_SLEEP mode.\n
 *        If the RTL907X_C is allowed to enter WHOLE_CHIP_SLEEP mode, when any port of RTL907X_C receives a sleep request and enters OP_LOCAL_SLEEP mode,\n
 *        the RTL907X_C will check if all the ports in the WholeChipSleepMask are in OP_LOCAL_SLEEP mode.\n
 *        If the RTL907X_C is allowed to enter WHOLE_CHIP_SLEEP mode and all the ports in the WholeChipSleepMask are in OP_LOCAL_SLEEP mode,\n
 *        the RTL907X_C will then enter WHOLE_CHIP_SLEEP mode.
 *
 * @param[in] unit Stacking unit (1 to 7).
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 ...
 * @retval Other error codes as defined in cvb_drv_indirect_sram_2bytes_get() and cvb_drv_indirect_sram_2bytes_set().
 *
 * @note Reference:
 *       - cvb_op_set_wholechip_sleep_portmap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue).
 */
RtkApiRet cvb_op_set_en_wholechip_sleep(uint32 unit, RtkEnable wholeChipOn)
{
    uint16    value16 = 0u;
    RtkApiRet retVal  = RT_ERR_OK;
    ...
    {
        /* Check input parameters */
        if((CVB_ENABLED != wholeChipOn) && (CVB_DISABLED != wholeChipOn))
        {
            retVal = -RT_ERR_INPUT;
            continue;
        }
...
/**
 * @brief Get the WholeChipSleep configuration for allowing or disallowing the RTL907X_C to enter WHOLE_CHIP_SLEEP mode.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 ...
 * @retval Other error codes as defined in cvb_drv_indirect_sram_2bytes_get().
 *
 * @note Reference:
 *       - cvb_op_set_en_wholechip_sleep(uint32 unit, RtkEnable wholeChipOn).
 */
RtkApiRet cvb_op_get_en_wholechip_sleep(uint32 unit, RtkEnable *pWholeChipOn)
{
    uint16    value16 = 0u;
    RtkApiRet retVal  = RT_ERR_OK;
    ...
    do
    {
        ...
        /* Get the bit[10] control the enabling of whole chip sleep setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_get(unit, CVB_OP_ACCEPT_SLEEP_CAP, &value16));
        /* The bit[10] of address CVB_OP_ACCEPT_SLEEP_CAP is the sleep enable bit */
 ...
 *        The RTL907X_C will enter WHOLE_CHIP_SLEEP mode if all the ports in the WholeChipSleepMask are in OP_LOCAL_SLEEP mode.\n
 *        The default value of WholeChipSleepMask is 0, meaning no port is in the WholeChipSleepMask.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[in] portMap The port map indicating selected ports in WholeChipSleepMask to be configured (0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert(), cvb_reg_read(), and cvb_reg_write().
 *
 * @note When any port of RTL907X_C receives a sleep request and enters OP_LOCAL_SLEEP mode,
 *       the RTL907X_C will check the WholeChipSleepMask to see if all the ports in it are in OP_LOCAL_SLEEP mode.
 *       If the ports in the WholeChipSleepMask are all in OP_LOCAL_SLEEP mode, RTL907X_C will enter WHOLE_CHIP_SLEEP mode.
 *       The WholeChipSleepMask cannot include ports 7, 8, 9, 10, 11, and 12.
 *
 * @note References:
 *       - cvb_op_set_en_wholechip_sleep(uint32 unit, RtkEnable wholeChipOn)
 *       - cvb_op_set_phy_sleep_cap(uint32 unit, RtkPmap portMap, RtkEnable sleepCap)
 *       - cvb_op_set_accept_sleep_req_cap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue)
 *       - cvb_op_set_txsleep_portmap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue)
 *       - cvb_op_port_send_sleep_request(uint32 unit, RtkPmap portMap)
 */
RtkApiRet cvb_op_set_wholechip_sleep_portmap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue)
{
    uint32    regValue = 0u;
    RtkPmap   dstPmap  = 0u;
    RtkPort   phyId    = 0u;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_reg_read(unit, CVB_OP_WHOLE_CHIP_SLEEP_MASK, &regValue));
        /* Get valid bits */
        regValue &= (CVB_RTK_T1PHY_PMAP_ALL);
        ...
        for(phyId = 0u; phyId <= CVB_RTK_PHY_ID_MAX; phyId++)
        {
            ...
            {
                regValue = regValue & (((uint32)(0x1u) << phyId) ^ CVB_REG_DATA_MASK);
            }
        }

        /* Write the updated PHY index mask back to the whole chip sleep register */
        CHK_FUN_CONTINUE(retVal, cvb_reg_write(unit, CVB_OP_WHOLE_CHIP_SLEEP_MASK, regValue));
    } while(0u == 1u);
 ...
 *        The RTL907X_C will enter WHOLE_CHIP_SLEEP mode if the ports configured in the WholeChipSleepMask are all in OP_LOCAL_SLEEP mode.\n
 *        The default value of WholeChipSleepMask is 0, meaning no port is in the WholeChipSleepMask.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 ...
 *                     - ENABLE: Belongs to
 * @param[out] pPortMap Pointer to the WholeChipSleepMask (0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval RT_ERR_OK on success.
 * @retval -RT_ERR_NULL_POINTER if the pointer *pPortMap is NULL.
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert() and cvb_reg_read().
 *
 * @note Reference:
 *       - cvb_op_set_wholechip_sleep_portmap(uint32 unit, RtkPmap portMap, RtkEnable cfgValue).
 */
RtkApiRet cvb_op_get_wholechip_sleep_portmap(uint32 unit, RtkEnable cfgValue, RtkPmap *pPortMap)
{
    ...
    do
    {
        ...
        /* Read the whole chip sleep register */
        CHK_FUN_CONTINUE(retVal, cvb_reg_read(unit, CVB_OP_WHOLE_CHIP_SLEEP_MASK, &wholechipmsk_tmp));
        if(CVB_ENABLED == cfgValue)
        {
            wholechipmsk_tmp = (wholechipmsk_tmp & CVB_RTK_T1PHY_PMAP_ALL);
        }
        else
        {
            wholechipmsk_tmp = (uint32)((~wholechipmsk_tmp) & CVB_RTK_T1PHY_PMAP_ALL);
        }
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert((RtkPmap)wholechipmsk_tmp, &dstPmap, CVB_EM_T1PHY_TO_USER));
        *pPortMap = dstPmap;
    } while(0u == 1u);
 ...
 *        When RTL907X_C is in global sleep mode, it can be woken up when detecting the configured LWAKE signal edge on the WAKE pin.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get() and cvb_drv_hw_indirect_set().
 *
 * @note The default is set as rising edge detection.
 */
RtkApiRet cvb_op_set_lwake_edge_sel(uint32 unit, uint16 lwakeEdge)
{
    uint32    regValue = 0u;
    RtkApiRet retVal   = RT_ERR_OK;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_LWAKE_EDGE << 1u), &regValue));
        if(OP_LWAKE_RISING_EDGE == lwakeEdge)
        {
            /* Set bit11 to 0 */
            regValue = regValue & (((uint32)(0x1u) << 11u) ^ CVB_REG_DATA_MASK);
        }
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (CVB_OP_LWAKE_EDGE << 1u), regValue));
    } while(0u == 1u);
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get().
 *
 * @note Reference:
 *       - cvb_op_set_lwake_edge_sel(uint32 unit, uint16 lwakeEdge).
 */
RtkApiRet cvb_op_get_lwake_edge_sel(uint32 unit, uint16 *pLwakeEdge)
{
    uint32    regValue = 0u;
    RtkApiRet retVal   = RT_ERR_OK;
    ...
    do
    {
        ...
        /* Get the current lwake edge setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_LWAKE_EDGE << 1), &regValue));
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 The stacking unit ID is used for indicating which switch is to be accessed.
 * @param[in] portMap lwake_wake_port port map (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert(), cvb_drv_hw_indirect_get(), and cvb_drv_hw_indirect_set().
 */
RtkApiRet cvb_op_set_lwake_wake_port(uint32 unit, RtkPmap portMap, RtkEnable cfgValue)
{
    uint32    regValue = 0u;
    RtkPmap   dstPmap  = 0u;
    RtkPort   phyId    = 0u;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_LWAKE_WAKE_PORT << 1u), &regValue));
        /* Get valid bits */
        regValue &= CVB_RTK_T1PHY_PMAP_ALL;
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (CVB_OP_LWAKE_WAKE_PORT << 1), regValue));
    } while(0u == 1u);
 ...
 *        If the RTL907x_C detects a local wakeup event, the ports in the lwake_wake_port port map will wake up after detecting an lwake event and might initiate a remote wakeup to its link partner.\n
 *        By default, all ports have the lwake_wake_port functionality enabled.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 The stacking unit ID is used for indicating which switch is to be accessed.
 * @param[out] pPortMap Return lwake_wake_port port map (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval RT_ERR_OK on success.
 * @retval -RT_ERR_NULL_POINTER if the pointer pPortMap is NULL.
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get() and cvb_drv_t1phy_pmap_convert().
 *
 * @note Reference:
 *       - cvb_op_set_lwake_wake_port(uint32 unit, RtkPmap portMap, uint8 cfgValue).
 */
RtkApiRet cvb_op_get_lwake_wake_port(uint32 unit, RtkEnable cfgValue, RtkPmap *pPortMap)
{
    uint32    value32 = 0u;
    RtkPmap   dstPmap = 0u;
    RtkApiRet retVal  = RT_ERR_OK;
    ...
    do
    {
        ...
        /* Get the current local wakeup to remote wakeup setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_LWAKE_WAKE_PORT << 1), &value32));
        if(CVB_ENABLED == cfgValue)
        {
            value32 &= (CVB_RTK_T1PHY_PMAP_ALL);
        }
        else
        {
            value32 = ((~value32) & CVB_RTK_T1PHY_PMAP_ALL);
        }
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert((RtkPmap)value32, &dstPmap, CVB_EM_T1PHY_TO_USER));
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[in] portMap The port map indicating the setting of WUP/WUR transmitting (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 */
RtkApiRet cvb_op_set_en_wupr_tx(uint32 unit, RtkPmap portMap, RtkEnable cfgValue)
{
    uint32    regValue = 0u;
    RtkPmap   dstPmap  = 0u;
    RtkPort   phyId    = 0u;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_EN_WUPR_TX << 1u), &regValue));
        /* Get valid bits */
        regValue &= CVB_RTK_T1PHY_PMAP_ALL;
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (CVB_OP_EN_WUPR_TX << 1u), regValue));
    } while(0u == 1u);
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[out] pPortMap Pointer to the port mask setting of WUP/WUR transmitting (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 *
 * @note Reference:
 *       - cvb_op_set_en_wupr_tx(uint32 unit, RtkPmap portMap, RtkEnable cfgValue)
 */
RtkApiRet cvb_op_get_en_wupr_tx(uint32 unit, RtkEnable cfgValue, RtkPmap *pPortMap)
{
    uint32    value32 = 0u;
    RtkPmap   dstPmap = 0u;
    RtkApiRet retVal  = RT_ERR_OK;
    ...
    do
    {
        ...
        /* Get the current transmitting WUP/WUR remote wakeup setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_EN_WUPR_TX << 1), &value32));
        if(CVB_ENABLED == cfgValue)
        {
            value32 &= (CVB_RTK_T1PHY_PMAP_ALL);
        }
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[in] portMap The port map indicating selected ports (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 */
RtkApiRet cvb_op_set_disable_rwake_rx(uint32 unit, RtkPmap portMap, uint8 cfgValue)
{
    uint32    regValue = 0u;
    RtkPmap   dstPmap  = 0u;
    RtkPort   phyId    = 0u;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_DIS_RWAKE << 1u), &regValue));
        /* Get valid bits */
        regValue &= CVB_RTK_T1PHY_PMAP_ALL;
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (CVB_OP_DIS_RWAKE << 1), regValue));
    } while(0u == 1u);
 ...
 *        By default, all ports allow remote wakeup detection, so get_cvb_op_disable_rwake_rx will return 0.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[out] pPortMap Pointer to the remote wakeup detection setting (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 *
 * @note Reference:
 *       - cvb_op_set_disable_rwake_rx(uint32 unit, RtkPmap portMap, uint8 cfgValue)
 */
RtkApiRet cvb_op_get_disable_rwake_rx(uint32 unit, RtkPmap *pPortMap)
{
    uint32    value32 = 0u;
    RtkPmap   dstPmap = 0u;
    RtkApiRet retVal  = RT_ERR_OK;
    ...
    do
    {
        ...
        /* Get the current disable remote wakeup detection setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_DIS_RWAKE << 1), &value32));
        /* Get valid bits */
        value32 &= CVB_RTK_T1PHY_PMAP_ALL;
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert((RtkPmap)value32, &dstPmap, CVB_EM_T1PHY_TO_USER));
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[in] portSel Port ID (CVB_RTK_USER_PORT_1ST to CVB_RTK_USER_PORT_6TH).\n
 *                    User port index of the selected port.
 * @param[out] pPortMap Port mask indicating forwarding remote wake setting (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_pmap_convert() and cvb_drv_hw_indirect_get().
 *
 * @note Reference:
 *       - cvb_op_set_portmap_fwd_rwake_to_port(uint32 unit, RtkPort portSel, RtkPmap portMap, RtkEnable cfgValue).
 */
RtkApiRet cvb_op_set_portmap_fwd_rwake_to_port(uint32 unit, RtkPort portSel, RtkPmap portMap, RtkEnable cfgValue)
{
    uint32    regLocal    = 0u;
    uint32    regValOp1   = 0u;
    uint32    regValOp2   = 0u;
    ...
    do
    {
        ...
        if((CVB_RTK_USER_PORT_1ST > portSel) || (CVB_RTK_USER_PORT_6TH < portSel))
        {
            retVal = -RT_ERR_INPUT;
            continue;
        }
        if(portMap != (portMap & CVB_RTK_USER_PMAP_WITHT1PHY))
        {
            retVal = -RT_ERR_INPUT;
            continue;
        }
        ...
        /* Translate the user port map to PHY index mask */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert(portMap, &dstPmap, CVB_EM_USER_TO_T1PHY));
        regLocal = cvb_g_sysPortToLocalPxCmd[cvb_g_userPort2sysPort[portSel]];
        /* Get the current forward pxcfg setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (regLocal << 1u), &regValue));
        for(phyId = 0u; phyId < (uint32)(cvb_g_userPort2T1phy[portSel]); phyId++)
        {
        ...
        regValue  = (regValOp1 | regValOp2) & (((uint32)0x1u << cvb_g_userPort2T1phy[portSel]) ^ 0x0000ffffu);
        /* Update the current setting according to the input */
        for(phyId = 0u; phyId <= CVB_RTK_PHY_ID_MAX; phyId++)
        {
            if(0u == (((uint32)0x1u << phyId) & dstPmap))
        ...
        regValOp1 = regValue >> ((uint32)cvb_g_userPort2T1phy[portSel] + 1u);
        regValOp2 = regValue & regValOpMsk;
        regValue  = (regValOp1 << (uint32)cvb_g_userPort2T1phy[portSel]) | regValOp2;
        /* Write back the current forward pxcfg setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (regLocal << 1u), regValue));
    } while(0u == 1u);
 ...
 *       - cvb_op_set_portmap_fwd_rwake_to_port(uint32 unit, RtkPort portSel, RtkPmap portMap, RtkEnable cfgValue).
 */
RtkApiRet cvb_op_get_portmap_fwd_rwake_to_port(uint32 unit, RtkPort portSel, RtkPmap *pPortMap)
{
    uint32    value32     = 0u;
    uint32    regValOp1   = 0u;
    uint32    regValOp2   = 0u;
    ...
    do
    {
        ...
        regLocal = cvb_g_sysPortToLocalPxCmd[cvb_g_userPort2sysPort[portSel]];
        /* Get the current forward pxcfg setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, ((uint32)regLocal << 1u), &value32));
        ...
        value32   = (regValOp1 | regValOp2) & (((uint32)0x1u << cvb_g_userPort2T1phy[portSel]) ^ 0xFFFFu);
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert((RtkPmap)value32, &dstPmap, CVB_EM_T1PHY_TO_USER));
        *pPortMap = (dstPmap & 0xFFFFu);
    } while(0u == 1u);
 ...
 *                      - 0: Config Index = 0
 *                      - 1: Config Index = 1
 * @param[in] portMap Wake-out forwarding port map (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).
 ...
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert(), cvb_drv_indirect_sram_2bytes_get(), and cvb_drv_indirect_sram_2bytes_set().
 *
 * @note In RTL907X_C, Config Index 0 is for GPIOB1, and Config Index 1 is for GPIOB2.
 */
RtkApiRet cvb_op_set_wakeo_portmap(uint32 unit, uint8 configIdx, RtkPmap portMap, RtkEnable cfgValue)
{
    uint32    cfgAddr = 0u;
    uint16    value16 = 0u;
    uint8     gpioId  = 0u;
    ...
    do
    {
        ...
        {
            gpioId = cvb_g_wakeoConfigIdxToGpioIdx[configIdx];
        }
        else
        {
            retVal = -RT_ERR_INPUT;
        ...
        {
            cfgAddr = CVB_OP_WAKEO_GPIOB1_PORTS;
        }
        if(OP_WAKEO_GPIOB2_ID == gpioId)
        {
            cfgAddr = CVB_OP_WAKEO_GPIOB2_PORTS;
        }
        /* Get the wake-out forwarding setting GPIO index */
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_get(unit, cfgAddr, &value16));
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_set(unit, cfgAddr, value16));
    } while(0u == 1u);
 ...
 * @param[out] pPortMap Wake-out forwarding port map (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 *
 * @note Reference:
 *       - cvb_op_set_wakeo_portmap(uint32 unit, uint8 configIdx, RtkPmap portMap, uint16 cfgValue).
 */
RtkApiRet cvb_op_get_wakeo_portmap(uint32 unit, uint8 configIdx, RtkEnable cfgValue, RtkPmap *pPortMap)
{
    uint16    value16 = 0u;
    RtkPmap   srcPmap = 0u;
    RtkPmap   dstPmap = 0u;
    ...
    do
    {
        ...
        if(OP_WAKEO_GPIOB1_ID == gpioId)
        {
            ...
            CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_get(unit, CVB_OP_WAKEO_GPIOB1_PORTS, &value16));
        }
        if(OP_WAKEO_GPIOB2_ID == gpioId)
        {
            /* get wake-out forwarding configuration of GPIOB2 */
            CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_get(unit, CVB_OP_WAKEO_GPIOB2_PORTS, &value16));
        }
        /* Return the corresponding configuration value based on whether the parameter cfgValue is enable or disable */
        if(CVB_DISABLED == cfgValue)
        {
 ...
 * @retval Other error codes as defined in cvb_drv_indirect_sram_1byte_set().
 */
RtkApiRet cvb_op_set_wakeo_pulse(uint32 unit, uint16 cfgValue)
{
    uint8     sramValue = 0u;
    RtkApiRet retVal    = RT_ERR_OK;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_1byte_set(unit, CVB_OP_WAKEO_PULSE_CFG, sramValue));
    } while(0u == 1u);
 ...
 * @retval Other error codes as defined in cvb_drv_indirect_sram_1byte_get().
 *
 * @note Reference:
 *       - cvb_op_set_wakeo_pulse(uint32 unit, uint16 cfgValue).
 */
RtkApiRet cvb_op_get_wakeo_pulse(uint32 unit, uint16 *pCfgValue)
{
    uint8     sramValue = 0u;
    RtkApiRet retVal    = RT_ERR_OK;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_1byte_get(unit, CVB_OP_WAKEO_PULSE_CFG, &sramValue));
...
/**
 * @brief Get the global OPFSM state of the RTL907X_C.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[out] pState Pointer to the global OPFSM state.\n
 *                    The global OPFSM state of RTL907X_C can be one of the following:
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get().
 */
RtkApiRet cvb_op_get_global_state(uint32 unit, uint16 *pState)
{
    uint32    regValue = 0u;
    RtkApiRet retVal   = RT_ERR_OK;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OCP_MR1 << 1u), &regValue));
...
/**
 * @brief Set the global OPFSM state of RTL907X_C.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 ...
 *
 * @note Use cases:
 *       - cvb_op_set_global_state(unit, OP_GLOB_STANDBY); Set global OPFSM state to STANDBY mode.
 *       - cvb_op_set_global_state(unit, OP_GLOB_NORMAL); Set global OPFSM state to NORMAL mode.
 *       - cvb_op_set_global_state(unit, OP_GLOB_WHOLE_CHIP_SLEEP); Set global OPFSM state to SLEEP mode.
 *       - Users cannot set the global OPFSM state to OP_GLOB_RESET or OP_GLOB_SAFETY.
 */
RtkApiRet cvb_op_set_global_state(uint32 unit, uint16 state)
{
    uint32    regValue  = 0u;
    uint32    globValue = 0u;
    RtkApiRet retVal    = RT_ERR_OK;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_GLOB_CMD << 1u), &regValue));
        switch(state)
        {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (CVB_OP_GLOB_CMD << 1u), regValue));
    } while(0u == 1u);
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[in] portId Port ID (CVB_RTK_USER_PORT_1ST to CVB_RTK_USER_PORT_6TH).\n
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get().
 */
RtkApiRet cvb_op_get_port_local_state(uint32 unit, RtkPort portId, uint16 *pState)
{
    uint32    regLocal = 0u;
    uint32    regValue = 0u;
    RtkApiRet retVal   = RT_ERR_OK;
    /* The mapping from the phy idx to local state OCP addr:         phy0    phy1    phy2    phy3    phy4    phy5    phy6    phy7    phy8    phy9    phy10   phy11 */
    const uint32 phyIdx2opLocalStateAddr[CVB_RTK_SYS_PORT_ID_MAX] = {0xB12u, 0xB14u, 0xB16u, 0xB18u, 0xB1Au, 0xB4Eu, 0xB50u, 0xB52u, 0xB54u, 0xB56u, 0xB58u, 0xB5Au};
    ...
    do
    {
        ...
        if((CVB_OP_T1_USER_PORT_MIN > portId) || (CVB_OP_T1_USER_PORT_MAX < portId))
        {
            retVal  = -RT_ERR_INPUT;
            *pState = (uint16)(0x0);
            continue;
        ...
        regLocal = phyIdx2opLocalStateAddr[cvb_g_userPort2T1phy[portId]];
 ...
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert() and cvb_drv_hw_indirect_set().
 */
RtkApiRet cvb_op_set_port_local_state(uint32 unit, RtkPort portId, uint16 state)
{
    uint32    regLocal = 0u;
    uint32    regValue = 0u;
    RtkApiRet retVal   = RT_ERR_OK;
    ...
    do
    {
        ...
        regLocal = cvb_g_sysPortToLocalCmd[cvb_g_userPort2sysPort[portId]];
 ...
 *                  - 0x20: OP_LOCAL_STANDBY
 *                  - 0x30: OP_LOCAL_NORMAL
 * @param[out] pPortMap Pointer to the port map indicating ports with the designated local OP state (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get().
 */
RtkApiRet cvb_op_get_portmap_local_state(uint32 unit, uint16 state, RtkPmap *pPortMap)
{
    uint32    regLocal   = 0u;
    uint32    regValue   = 0u;
    ...
    do
    {
        ...
        *pPortMap = 0u;
        for(userPortId = CVB_OP_T1_USER_PORT_MIN; userPortId <= CVB_RTK_USER_PORT_WITHT1PHY_ID_MAX; userPortId++)
        {
            ...
            regLocal = phyIdx2opLocalStateAddr[cvb_g_userPort2T1phy[userPortId]];
            /* Get the port's local OPFSM state by reading the value of the specific register */
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (regLocal << 1u), &regValue));
 ...
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert() and cvb_drv_hw_indirect_set().
 */
RtkApiRet cvb_op_set_portmap_local_state(uint32 unit, RtkPmap portMap, uint16 state)
{
    uint32    regLocal   = 0u;
    uint32    regValue   = 0u;
    RtkPmap   dstPmap    = 0u;
    ...
    do
    {
        ...
        for(userPortId = CVB_OP_T1_USER_PORT_MIN; userPortId <= CVB_RTK_USER_PORT_WITHT1PHY_ID_MAX; userPortId++)
        {
            ...
            /* Set the PHY local state of the input portId */
            regLocal = cvb_g_sysPortToLocalCmd[cvb_g_userPort2sysPort[userPortId]];
            /* Set the port's local OPFSM state by writing the value to the specific register */
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (regLocal << 1u), regValue));
        }
    } while(0u == 1u);
 ...
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert(), cvb_drv_hw_indirect_set(), cvb_reg_read(), and cvb_reg_write().
 *
 ...
 *       - cvb_op_set_phy_sleep_cap(uint32 unit, RtkPmap portMap, RtkEnable sleepCapOn).
 */
RtkApiRet cvb_op_port_send_sleep_request(uint32 unit, RtkPmap portMap)
{
    uint32    regOffset  = 0u;
    uint32    regValue   = 0u;
    uint32    readClear  = 0u;
    ...
    do
    {
        ...
        for(userPortId = CVB_RTK_USER_PORT_1ST; userPortId <= CVB_RTK_USER_PORT_WITHT1PHY_ID_MAX; userPortId++)
        {
            if(0u == (((uint32)1u << userPortId) & portMap))
            {
                continue;
            ...
            sysPort   = cvb_g_userPort2sysPort[userPortId];
            regOffset = ((uint32)(cvb_g_sysPortToOcpAddr[sysPort] | CVB_OCP_SLPCR)) << 1u;
            CHK_FUN_CONTINUE(retVal, cvb_reg_read(unit, regOffset, &regValue));

            /* Need to clear the WUR and WUP flag in advance. Otherwise, the sleep request might not be sent successfully */
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_WUR_FLAG << 1u), &readClear));
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_WUP_FLAG << 1u), &readClear));
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, CVB_OP_WUR_WUP_FLAG, 0x0u));
            /* Set bit[9] to 1 */
            CHK_FUN_CONTINUE(retVal, cvb_reg_bit_set(unit, regOffset, OP_LOC_SLP_REQ_BIT, 1u));
        }
    } while(0u == 1u);
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[out] pRxSlpFlag Pointer to the receiving sleep flag (0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get(), cvb_drv_hw_indirect_set(), and cvb_drv_t1phy_pmap_convert().
 */
RtkApiRet cvb_op_get_rx_sleep_flag(uint32 unit, uint16 *pRxSlpFlag)
{
    uint32    value32 = 0u;
    uint32    rxSleep = 0u;
    RtkPmap   dstPmap = 0u;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, CVB_OP_SLEEP_EVENT_FLAG, &value32));
        /* Get the receiving sleep request flag */
        rxSleep = (value32 & (CVB_OP_RX_SLP_EVENT_MASK)) >> CVB_OP_RX_SLP_EVENT_OFFSET;
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert((RtkPmap)rxSleep, &dstPmap, CVB_EM_T1PHY_TO_USER));
        *pRxSlpFlag = (uint16)(dstPmap);
        /* Clear the bits of receiving sleep request flag */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, CVB_OP_SLEEP_EVENT_FLAG, (value32 & (~CVB_OP_RX_SLP_EVENT_MASK))));
    } while(0u == 1u);
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[out] pSlpFailFlag Pointer to the sleep handshake fail flag (0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get(), cvb_drv_hw_indirect_set(), and cvb_drv_t1phy_pmap_convert().
 */
RtkApiRet cvb_op_get_sleep_fail_flag(uint32 unit, uint16 *pSlpFailFlag)
{
    uint32    value32   = 0u;
    uint32    sleepFail = 0u;
    RtkPmap   dstPmap   = 0u;
    ...
    do
    {
        ...
        sleepFail = (value32 & (CVB_OP_SLP_FAIL_EVENT_MASK)) >> CVB_OP_SLP_FAIL_EVENT_OFFSET;
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert((RtkPmap)sleepFail, &dstPmap, CVB_EM_T1PHY_TO_USER));
        *pSlpFailFlag = (uint16)(dstPmap);
        /* Clear the bits of sleep handshake fail indication flag */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, CVB_OP_SLEEP_EVENT_FLAG, (value32 & (~CVB_OP_SLP_FAIL_EVENT_MASK))));
    } while(0u == 1u);
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[out] pSlpSuccessFlag Pointer to the sleep handshake success flag (0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get(), cvb_drv_hw_indirect_set(), and cvb_drv_t1phy_pmap_convert().
 */
RtkApiRet cvb_op_get_sleep_success_flag(uint32 unit, uint16 *pSlpSuccessFlag)
{
    uint32    value32      = 0u;
    ...
    do
    {
        ...
        sleepSuccess = (value32 & (CVB_OP_SLP_SUCCESS_EVENT_MASK)) >> CVB_OP_SLP_SUCCESS_EVENT_OFFSET;
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert((RtkPmap)sleepSuccess, &dstPmap, CVB_EM_T1PHY_TO_USER));
        *pSlpSuccessFlag = (uint16)(dstPmap);
        /* Clear the bits of sleep handshake success indication flag */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, CVB_OP_SLEEP_EVENT_FLAG, (value32 & (~CVB_OP_SLP_SUCCESS_EVENT_MASK))));
    } while(0u == 1u);
 ...
 *
 * @note Reference:
 *       - cvb_op_set_en_wupr_tx(uint32 unit, RtkPmap portMap, uint8 cfgValue).
 */
RtkApiRet cvb_op_port_send_rwake(uint32 unit, RtkPmap portMap)
{
    uint32    regLocal   = 0u;
    uint32    regValue   = 0u;
    RtkPmap   dstPmap    = 0u;
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 The stacking unit ID is used for indicating which switch is to be accessed.
 * @param[out] pPortMap The WUP flag (0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get(), cvb_drv_hw_indirect_set(), and cvb_drv_t1phy_pmap_convert().
 */
RtkApiRet cvb_op_get_wup_flag(uint32 unit, RtkPmap *pPortMap)
{
    uint32    value32     = 0u;
    uint32    wakeoCfg    = 0u;
    uint32    pulseCfg    = 0u;
    ...
    do
    {
        ...
        /* Read 32 bits data from CVB_OP_WAKEO_GPIOB1_PORTS, including CVB_OP_WAKEO_GPIOB1_PORTS(bit[15:0]) and CVB_OP_WAKEO_GPIOB2_PORTS(bit[31:16]) */
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_get(unit, CVB_OP_WAKEO_GPIOB1_PORTS, &wakeoCfg));
        /* Get Wake-out forwarding pulse config */
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_get(unit, CVB_OP_WAKEO_PULSE_CFG, &pulseCfg));
        /* Get the g_Opfsm_VirInterrupt_Mode */
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_get(unit, CVB_OP_VIR_INTR_MODE, &virIntrMode));
        /* The valid bit is 8 */
        virIntrMode &= 0xFFu;

        if(((0u != wakeoCfg) && (OP_WAKEO_PULSETYPE_UNKNOWN != pulseCfg)) || (CVB_OP_VIRINTERUPT_POLLING_ENABLE == virIntrMode))
        {
            /* Wake-out forwarding supported by ROM code is enabled */
            /* bit[15:0] of CVB_OP_WUR_WUP_FLAG is CVB_OP_WUP_FLAG */
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, CVB_OP_WUR_WUP_FLAG, &value32));
            /* Clear WUP-flag bit[15:0] in CVB_OP_WUR_WUP_FLAG, set to 0 */
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, CVB_OP_WUR_WUP_FLAG, (value32 & 0xFFFF0000u)));
            /* Read WUP-flag */
            value32 &= 0x0000FFFFu;
        }
        else
        {
            ...
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_WUP_FLAG << 1u), &value32));
        }
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert((RtkPmap)value32, &dstPmap, CVB_EM_T1PHY_TO_USER));
        *pPortMap = (dstPmap & 0xFFFFu);
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 The stacking unit ID is used for indicating which switch is to be accessed.
 * @param[out] pPortMap The WUR flag (0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get(), cvb_drv_hw_indirect_set(), and cvb_drv_t1phy_pmap_convert().
 */
RtkApiRet cvb_op_get_wur_flag(uint32 unit, RtkPmap *pPortMap)
{
    uint32    value32     = 0u;
    uint32    wakeoCfg    = 0u;
    uint32    pulseCfg    = 0u;
    ...
    do
    {
        ...
        {
            /* Wake-out forwarding supported by ROM code is enabled */
            /* bit[31:16] of CVB_OP_WUR_WUP_FLAG is CVB_OP_WUR_FLAG */
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, CVB_OP_WUR_WUP_FLAG, &value32));
            /* Clear WUR-flag bit[31:16] in CVB_OP_WUR_WUP_FLAG, set to 0 */
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, CVB_OP_WUR_WUP_FLAG, (value32 & (0x0000FFFFu))));
            /* Read WUR-flag */
        ...
        else
        {
            ...
            CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_WUR_FLAG << 1u), &value32));
        }
        /* Translate the PHY index mask to user port map */
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert((RtkPmap)value32, &dstPmap, CVB_EM_T1PHY_TO_USER));
        *pPortMap = (dstPmap & 0xFFFFu);
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get() and cvb_drv_hw_indirect_set().
 */
RtkApiRet cvb_op_wakeflag_clearall(uint32 unit)
{
    uint32    readClear = 0u;
    RtkApiRet retVal    = RT_ERR_OK;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_REC_FLAG << 1), &readClear));
        /* Read-clear Power-on Reg */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_PWON_FLAG << 1), &readClear));
    } while(0u == 1u);
...
/**
 * @brief Set CVB_OP_OTHER_INTR_MSK port map.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 The stacking unit ID is used for indicating which switch is to be accessed.
 * @param[in] cfgValue Set value (0 to 0xF).\n
 *                     Each bit configuration of CVB_OP_OTHER_INTR_MSK:
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get() and cvb_drv_hw_indirect_set().
 */
RtkApiRet cvb_op_set_event_ext_intr_msk(uint32 unit, uint16 cfgValue)
{
    uint32    regValue = 0u;
    RtkApiRet retVal   = RT_ERR_OK;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_OTHER_INTR_MSK << 1u), &regValue));
        /* Get valid bits */
        regValue &= 0xFFFFu;
        regValue = (regValue & 0x0FFFu) | ((cfgValue << 12u) & 0xF000u);
        /* Write the updated current setting back to the register */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (uint32)CVB_OP_OTHER_INTR_MSK << 1u, regValue));
    } while(0u == 1u);
...
/**
 * @brief Get CVB_OP_OTHER_INTR_MSK port map.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 The stacking unit ID is used for indicating which switch is to be accessed.
 * @param[out] pCfgValue Get CVB_OP_OTHER_INTR_MSK (0 to 0xF).\n
 *                       Each bit of CVB_OP_OTHER_INTR_MSK:
 ...
 * @retval Other error codes as defined in cvb_drv_hw_indirect_get().
 */
RtkApiRet cvb_op_get_event_ext_intr_msk(uint32 unit, uint16 *pCfgValue)
{
    uint32    regValue = 0u;
    RtkApiRet retVal   = RT_ERR_OK;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_OTHER_INTR_MSK << 1), &regValue));
        /* Get valid bits */
        regValue &= 0xFFFFu;
...
/**
 * @brief Allow or disallow the RTL907X_C to output an interrupt signal when specific sleep events occur.\n
 *        The sleep events include:\n
 *        1. Receive sleep request\n
 *        2. Sleep handshake fail\n
 *        3. Enter sleep mode
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[in] extIntrOn External interrupt triggered by sleep events configuration (DISABLE or ENABLE).\n
 *                      Set the enable bit of external interrupt triggered by sleep events:
 *                      - DISABLE: Disable the external interrupt triggered by sleep events
 *                      - ENABLE: Enable the external interrupt triggered by sleep events
 *
 * @retval RT_ERR_OK on success.
 * @retval -RT_ERR_INPUT if the input value of extIntrOn is invalid.
 * @retval Other error codes as defined in cvb_drv_indirect_sram_2bytes_get() and cvb_drv_indirect_sram_2bytes_set().
 */
RtkApiRet cvb_op_set_sleep_event_intr_en(uint32 unit, RtkEnable extIntrOn)
{
    uint16    value16 = 0u;
    RtkApiRet retVal  = RT_ERR_OK;
    ...
    {
        /* Check input parameters */
        if((CVB_ENABLED != extIntrOn) && (CVB_DISABLED != extIntrOn))
        {
            retVal = -RT_ERR_INPUT;
            continue;
        }

        /* Get the bit[12] control the enabling of external sleep interrupt triggered by sleep events */
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_get(unit, CVB_OP_ACCEPT_SLEEP_CAP, &value16));
        if(CVB_ENABLED == extIntrOn)
        {
            value16 = value16 | (((uint16)0x1u) << OP_SLP_EXT_INTR_CTRL_BIT);
        }
        else
        {
            value16 = value16 & ((((uint16)0x1u) << OP_SLP_EXT_INTR_CTRL_BIT) ^ 0xFFFFu);
        }
        /* Write the updated value back to the register */
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_set(unit, CVB_OP_ACCEPT_SLEEP_CAP, value16));
    } while(0u == 1u);
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 Stacking unit ID is used to indicate which switch is to be accessed.
 * @param[out] pExtIntrOn Pointer to the external interrupt triggered by sleep events configuration (DISABLE or ENABLE).\n
 *                        Gets the enable bit of external interrupt triggered by sleep events:
 *                        - DISABLE: Disable the external interrupt triggered by sleep events
 *                        - ENABLE: Enable the external interrupt triggered by sleep events
 *
 * @retval RT_ERR_OK on success.
 * @retval -RT_ERR_NULL_POINTER if the pointer *pExtIntrOn is NULL.
 * @retval Other error codes as defined in cvb_drv_indirect_sram_2bytes_get().
 */
RtkApiRet cvb_op_get_sleep_event_intr_en(uint32 unit, RtkEnable *pExtIntrOn)
{
    uint16    value16 = 0u;
    RtkApiRet retVal  = RT_ERR_OK;
    ...
    do
    {
        ...
        /* Get the bit[12] control the enabling of external interrupt triggered by sleep events */
        CHK_FUN_CONTINUE(retVal, cvb_drv_indirect_sram_2bytes_get(unit, CVB_OP_ACCEPT_SLEEP_CAP, &value16));
        /* The bit[12] of address CVB_OP_ACCEPT_SLEEP_CAP is the extern */
        value16     = (value16 >> OP_SLP_EXT_INTR_CTRL_BIT) & 0x1u;
        *pExtIntrOn = (RtkEnable)value16;
    } while(0u == 1u);
 ...
 *        Users can get the WUR flag indicating which port received WUR by using the function cvb_op_get_wur_flag(uint16 *getValue).\n
 *        The default WUR interrupt mask is all 1.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 The stacking unit ID is used for indicating which switch is to be accessed.
 * @param[in] portMap The WUR interrupt mask (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 *
 * @note Reference:
 *       - cvb_op_get_wur_ext_intr_msk(uint32 unit, uint16 *getValue)
 */
RtkApiRet cvb_op_set_wur_ext_intr_msk(uint32 unit, RtkPmap portMap, uint8 cfgValue)
{
    uint32    regValue = 0u;
    RtkPmap   dstPmap  = 0u;
    RtkPort   phyId    = 0u;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_t1phy_pmap_convert(portMap, &dstPmap, CVB_EM_USER_TO_T1PHY));
        /* Get the current setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_WUR_INTR_MSK << 1), &regValue));
        /* Get valid bits */
        regValue &= CVB_RTK_T1PHY_PMAP_ALL;
        /* Update the current setting according to the input */
        for(phyId = 0u; phyId <= CVB_RTK_PHY_ID_MAX; phyId++)
        ...
        /* Write the updated current setting back to the register */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (CVB_OP_WUR_INTR_MSK << 1u), regValue));
    } while(0u == 1u);
 ...
 *                     - Mask: 1
 *                     - Unmask: 0
 * @param[out] pPortMap The WUR interrupt mask (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 * @retval RT_ERR_OK on success.
 * @retval -RT_ERR_NULL_POINTER if the input pointer pPortMap is NULL.
 * @retval Other error codes as defined in cvb_drv_t1phy_pmap_convert() and cvb_drv_hw_indirect_get().
 *
 * @note The PHY of ports 7, 8, 9, 10, 11, and 12 cannot be in sleep mode.
 *
 * @note Reference:
 *       - cvb_op_set_wur_ext_intr_msk(uint32 unit, RtkPmap portMap, uint8 cfgValue).
 */
RtkApiRet cvb_op_get_wur_ext_intr_msk(uint32 unit, uint8 cfgValue, RtkPmap *pPortMap)
{
    uint32    value32 = 0u;
    RtkPmap   dstPmap = 0u;
    RtkApiRet retVal  = RT_ERR_OK;
    ...
    do
    {
        ...
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_WUR_INTR_MSK << 1), &value32));
        if(1u == cfgValue)
        {
            value32 &= (CVB_RTK_T1PHY_PMAP_ALL);
        }
 ...
 *        Users can get the WUP flag indicating which port received WUP by using the function cvb_op_get_wup_flag(uint16 *getValue).\n
 *        The default WUP interrupt mask is all 1.
 *
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 The stacking unit ID is used for indicating which switch is to be accessed.
 * @param[in] portMap The WUP interrupt mask (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 *
 * @note Reference:
 *       - cvb_op_get_wup_ext_intr_msk(uint32 unit, uint16 *getValue)
 */
RtkApiRet cvb_op_set_wup_ext_intr_msk(uint32 unit, RtkPmap portMap, uint8 cfgValue)
{
    uint32    regValue = 0u;
    RtkPmap   dstPmap  = 0u;
    RtkPort   phyId    = 0u;
    ...
    do
    {
        ...
        dstPmap = (dstPmap << 2u);
        /* Get the current setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_WUP_INTR_MSK << 1u), &regValue));
        /* Get valid bits */
        regValue &= ((CVB_RTK_T1PHY_PMAP_ALL << 2u) | ((1u << 2u) - 1u));
        for(phyId = 0u; phyId <= (CVB_RTK_PHY_ID_MAX + 2u); phyId++)
        {
            if(0u == (((uint32)0x1u << phyId) & dstPmap))
            {
                continue;
        ...
        /* Write the updated current setting back to the register */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_set(unit, (CVB_OP_WUP_INTR_MSK << 1u), regValue));
    } while(0u == 1u);
 ...
 * @param[in] unit Stacking unit (1 to 7).\n
 *                 The stacking unit ID is used for indicating which switch is to be accessed.
 * @param[out] pPortMap The WUP interrupt mask (0x0 to CVB_RTK_USER_PMAP_WITHT1PHY).\n
 ...
 *
 * @note Reference:
 *       - cvb_op_set_wup_ext_intr_msk(uint32 unit, RtkPmap portMap, uint8 cfgValue).
 */
RtkApiRet cvb_op_get_wup_ext_intr_msk(uint32 unit, uint8 cfgValue, RtkPmap *pPortMap)
{
    uint32    value32   = 0u;
    uint32    regValTmp = 0u;
    RtkPmap   dstPmap   = 0u;
    ...
    do
    {
        ...
        /* Get the current setting */
        CHK_FUN_CONTINUE(retVal, cvb_drv_hw_indirect_get(unit, (CVB_OP_WUP_INTR_MSK << 1), &regValTmp));
        /* Valid bits is bit2 to bit7 of value32 */
        value32 = regValTmp & (CVB_RTK_T1PHY_PMAP_ALL << 2u);
