...
static int rtk9071_port_link_get(struct hw_device *dev, uint32_t port,link_status_e *link_status)
{
    int ret = 0;
    PhyLinkStatus_t PhyLinkStatus = {0};
    UnitChip_t     unitChip = {0};
    struct rtl9071_private *priv = (struct rtl9071_priv *)dev->priv;
    unitChip.chip = CHIP_907XC_VB,
    unitChip.unit = priv->unit;
...
#ifdef WTSWITCH_DBG
    printf("port %d link status:%d. ret:%d\n",port,*link_status,ret);
#endif
    return ret;
}
...
static RtkApiRet platform_pcie_read(uint32_t regAddr, uint32_t *value)
{
    ...
    else
    {
        ...
        {
            cmd_data.periph_id =  BSP_PERIPH_SWITCH;
            cmd_data.cmd =  RTL_CMD_REG_GET;
            cmd.type    = SWC_CMD_TYPE_32;
            cmd.regAddr = regAddr | g_pciePage;
            cmd_data.buf =(unsigned long long)&cmd;
            cmd_data.len =sizeof(rtase_swc_cmd_t);
            if(ioctl(bsp_io_fd,BSP_IO_IOCTL_CMD, &cmd_data) != 0)
            {
#ifdef DEBUG_PCIE_INTERFACE
...
static int rtl9071_reg_read(struct hw_device *device,uint32_t regAddr, uint32_t *value)
{
    return platform_pcie_read(regAddr, value);
}
...
static int rtl9071_reg_write(struct hw_device *device,uint32_t regAddr, uint32_t value)
{
    return platform_pcie_write(regAddr, value);
}
...
static int rtk9071_sdk_init(struct hw_device *dev)
{
    ...
    //start mib
    ret |= rtk_stat_start_all(unitChip);


    return ret;
}

static int rtk9071_sdk_deinit(struct hw_device *device)
{
    return 0;
}
...
{
    .sdk_init = (void *)rtk9071_sdk_init,
    .sdk_deinit = (void *)rtk9071_sdk_deinit,
    .port_link_set = rtk9071_port_link_set,
    .port_link_get = (void *)rtk9071_port_link_get,
    ...
    .switch_reg_read = (void *)rtl9071_reg_read,
    .switch_reg_write = (void *)rtl9071_reg_write,
...
{
    struct rtl9071_private *rtl9071_priv = NULL;
    cJSON *unit = NULL;

    rtl9071_priv = (struct rtl9071_private *)malloc(sizeof(struct rtl9071_private));
    if(rtl9071_priv == NULL)
    {
        HW_ERR("malloc mem error\n, %s %d",__FUNCTION__,__LINE__);
        return -1;
    }
    unit = cJSON_GetObjectItem(dev->root,"unit");
    if(unit == NULL)
    {
        HW_ERR("switch get unit error. %s %d\n",__FUNCTION__, __LINE__);
        return -1;
    }

    rtl9071_priv->unit = unit->valueint;
    HW_INFO("unit %d. %s %d\n",rtl9071_priv->unit,__FUNCTION__, __LINE__);
    dev->priv = rtl9071_priv;
    HW_INFO("rtl9071 probe\n");
    return 0;
}
static int rtl9071_remove(struct hw_device *dev)
{
    if(dev->priv)
    {
        free(dev->priv);
